...
import api from '@/app/lib/axios' // Importa a instância configurada do Axios
import {
  Carousel,
  CarouselContent,
  CarouselItem,
...
export default function Login() {
  ...
  const onSubmit = async (data: LoginFormValues) => {
    try {
      ...
      // Use a instância API com interceptores configurados
      const response = await api.post('/login', data)
      
      // Se chegou aqui, o login foi bem-sucedido (o interceptor já lida com erros)
      ...
      // O interceptor já tratou os erros específicos, então este bloco
      // só é executado para qualquer tratamento adicional necessário
      
      // Se precisar fazer algo específico com o erro no componente de login
      // pode fazê-lo aqui, mas não é necessário mostrar outra mensagem de erro
      
      // Evitamos logar erros 401 no console, pois são esperados
      if (error !== 401) {
